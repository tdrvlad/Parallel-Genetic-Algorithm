...
if my_id != 0:
	#Prossesors will simulate evolution in remote contexts - "islands"
	...
	#Main processor will wait for the other processors to fill up data of evolved populations
    
    hash_received = np.zeros(processors)
    fitness_evolutions = []
    population = []

    #main processor will read communications untill all the data from all the other processors has been transmitted
            ...
            if os.path.exists(file_name):
                ...
                if os.path.getsize(file_name) > 0 and hash_received[i] == 0:
                    ...
                    with open(file_name,"rb") as file:
	                    ...
	                    #Extracting data of interest: selected individuals and evolution history for later plotting
	                    (individuals,evolution) = pickle.load(file)
	                    population += individuals
	                    fitness_evolutions.append(evolution)

	                    #Printing some relevant intermediate data -  average fitness of the selected individuals from each island
	                    avg_fit = 0
	...
	#removing the files after the data has been read
    for i in range(1,processors):
    	file_name = file_name = "Island_" + str(i) +".txt"
    	if os.path.exists(file_name):
    		os.remove(file_name)
    ...
    #the second phase evolution history is common
